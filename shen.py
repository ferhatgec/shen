...
class shen:
    ...
    def highlight(self, data: str, syntax_rule: shen_syntax_rule) -> [str]:
                                    ...
                                    if len(val_data) >= 1:
                                        if ch == syntax_rule.capture_through.get(self.__capture_ch).ends_with:
                                            if data.include_current_char_coloring:
                                                self.tokens.append(f'\x1b[38;2;{data.r};{data.g};{data.b}m{self.__capture_ch}{val_data[0]}{ch}\x1b[0m')
                                            else:
                                                self.tokens.append(f'{self.__capture_ch}\x1b[38;2;{data.r};{data.g};{data.b}m{val_data[0]}\x1b[0m{ch}')
                                        else:
                                            if data.include_current_char_coloring:
                                                self.tokens.append(
                                                    f'\x1b[38;2;{data.r};{data.g};{data.b}m{ch}{val_data[0]}{ch}\x1b[0m')
                                            else:
                                                self.tokens.append(
                                                    f'{ch}\x1b[38;2;{data.r};{data.g};{data.b}m{val_data[0]}\x1b[0m{ch}')
                                else:
                                    self.__collect = False
                                    self.current_token += ch
                                    continue
            ...
            match ch:
                case ch if (ch in syntax_rule.push_tokens) or (self.current_token in syntax_rule.push_tokens):
                    if len(self.current_token) > 0:
                        ...
                        if data is not None:
                            if data.enable_regex:
                                ...
                                if len(val) >= 1:
                                    self.tokens.append(f'\x1b[38;2;{data.r};{data.g};{data.b}m{val[1]}\x1b[0m')
                            else:

                                self.tokens.append(f'\x1b[38;2;{data.r};{data.g};{data.b}m{self.current_token}\x1b[0m')
                        else:
                            self.tokens.append(self.current_token)
                    ...
                    if ch not in syntax_rule.ignored_tokens and not self.__collect:
                        ...
                        if data is not None:
                            self.tokens.append(f'\x1b[38;2;{data.r};{data.g};{data.b}m{ch}\x1b[0m')
                        else:
